...
export const getNeoResponse = async (message: string, history: { role: string, parts: { text: string }[] }[]) => {
  if (!apiKey) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "Você é o NEO, o Agente de Inteligência Artificial da NEOOH. Você é inovador, tecnológico e focado em soluções de mídia Out of Home (OOH) e Digital Out of Home (DOOH). Suas respostas devem ser CURTAS, DIRETAS e PRECISAS. Use o Google Maps para fornecer informações precisas sobre localizações, especialmente sobre a NEOOH em São Paulo. Use o site https://neooh.com.br para buscar informações sobre a empresa. Seja prestativo, profissional e use um tom futurista.",
        tools: [{ googleMaps: {} }]
      },
    });
